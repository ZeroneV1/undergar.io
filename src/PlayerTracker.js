...
PlayerTracker.prototype.getScale = function() {
    this._score = 0; // Reset to prevent leaderboard bugs
    var scale = 0; // Reset to prevent viewbox issues

    // If the player is a spectator, set their mass to 5000
    if (this.spectate && !this.freeRoam) {
        var spectatorMass = 100000;
        var spectatorSize = Math.sqrt(spectatorMass * 100); // Convert mass to size
        scale = spectatorSize; // Set the scale to the calculated size
        this._score = spectatorMass; // Set the score to the spectator mass
    } else {
        // Normal player scale calculation
        ...
        if (!scale) {
            scale = this._score = 0.4; // Default scale if no cells are present
        } else {
            scale = Math.pow(Math.min(64 / scale, 1), 0.4); // Normalize scale
        }
    }

    // Increase spectator cell size (if applicable)
    if (this.spectate && !this.freeRoam) {
        scale *= 3.5; // Adjust multiplier to make spectator cell larger
    }
